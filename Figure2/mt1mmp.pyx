...
def cpm(list pottsElements, list cellArr, list contactEnergy, list camElements, list volumeElements, list surfaceElements, list mt1Elements, list imageProps):
    ...
    cdef:
        str includeMembraneModel = mt1Elements[0]["UseMembraneModel"]
        int membranemt1 = mt1Elements[1]["MembraneMT1"]
        double lambdaM = mt1Elements[1]["LambdaMT1"]     
    ###########################################
        ...
        for i in xrange(0, (noLattice+1)):
            ...
            if i == noLattice:
                ...
                if includeMembraneModel == "Yes":
                    ...
                    ax.matshow(grid, cmap='Purples', interpolation='none', origin='lower', vmin=0.0, vmax=grid.max())

                
                ax.set_axis_off()
                
                ##--- To show the colorbar of MT1-MMP --##
                ...
                if includeMembraneModel == "Yes":
                    ...
                    ###===== Membrane MT1-MMP =====###
                    membranemt1 = mt1Elements[1]["MembraneMT1"]
                    alpha = 2.675 #-- for Max MT1MMP = 16; 
                    membranemt1 = int(alpha * activemt1)
                    mt1Elements[1]["MembraneMT1"] = membranemt1
