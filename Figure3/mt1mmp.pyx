...
######################################################
###===== Implementation of the Membrane Model =====###
######################################################
@cython.boundscheck(False)
@cython.wraparound(False)
cdef int gmfunc((int, int) targetpx, (int, int) sourcepx, np.ndarray[DTYPE_t, ndim=2] grid, np.ndarray[DTYPE_t, ndim=2] membrane, list pottsElements):
    ...
    for m1 in range(lengs3):
        gs4.append(membrane[gs3[m1]]) #--- list ---#
    ...
    for m2 in range(lengt3):
        gt4.append(membrane[gt3[m2]]) #--- list ---#
...
@cython.wraparound(False)
@cython.cdivision(True)
def cpm(list pottsElements, list cellArr, list contactEnergy, list camElements, list volumeElements, list surfaceElements, list mt1Elements, list imageProps):
    ...
    cdef:
        ...
        double Jcm = contactEnergy[1]["Value"]
        double Jcc = contactEnergy[2]["Value"]
        int dyntime = contactEnergy[3]["DynamicTime"]
    #########################################
    ...
    ###===== Membrane MT1-MMP elements =====###
    cdef:
        int membranemt1 = mt1Elements[0]["MembraneMT1"]
        double lambdaM = mt1Elements[0]["LambdaMT1"]
    ###########################################
    ...
    ###===== Initialization for the Intracellular MT1-MMP dynamics =====###        
    cdef:
        double h=0.1, x_init=0.0, xinit, ts, alpha, activemt1
        int nsteps = 10 
        double deltaHMembrane
        Py_ssize_t ii
    ...
    cdef:
        ...
        np.ndarray[double, ndim=1] y_init = np.array([5.5, 1.0], dtype=np.double)
    ...
    ###===== Options for Displaying and Saving Images =====#
    cdef:
        str showImages = imageProps[0]["DisplayImages"]
        str saveImages = imageProps[1]["SaveImages"]
        int save_each = imageProps[1]["SaveMCS"]  
    ########################################################     
    ...
    for j in xrange(0, (mcs+1)):
        ...
        for i in xrange(0, (noLattice+1)):
            ...
            if idxt != idxs: #-- if target pixel not equal source pixel --#
                ...
                #-- New cell volume after the copy attempt --#
                if idxs == 0: #-- if the source pixel is medium --#
                    cellVolNew = cellVol - 1 #-- Cell volume decreases due to the copy attempt --#
                else:         #-- if the source pixel is NOT medium --#
                    cellVolNew = cellVol + 1 #-- Cell volume increases due to the copy attempt --#

                #-- New cell area after the copy attempt --#
                ...
                if j >= dyntime:
                    ##===== Membrane Model =====###
                    deltaHMembrane = (lambdaM/membranemt1)*gmfunc(targetpx, sourcepx, grid, act, pottsElements)
                    #-- The success probability of the copy attempt is biased by subtracting
                    #-- deltaHMembrane from deltaH:
                    deltaH = deltaH - deltaHMembrane


                if connectpixels(targetpx, grid, xMax, yMax, BC) > 2:
                    deltaH += 10000000000.0
            ...
            if i == noLattice:
                ...
                if j >= dyntime:
                    ...
                    ##--- Use These to Include the Membrane Model
                    ax.matshow(grid, cmap='Blues', interpolation='none', origin='lower', vmin=0.0, vmax=grid.max())
                    ax.matshow(act, interpolation='nearest', cmap='jet', origin='lower', alpha=0.4, vmin=0, vmax=act.max())

                    #-- Choose other colormaps for membrane dynamics --#
                    # ax.matshow(act, interpolation='nearest', cmap='turbo', origin='lower', alpha=0.5, vmin=0, vmax=20)
                ...
                #-- Increase cell-matrix adhesion over time (MCS) --#
                if j > 50:
                    Jcm = 5.0
                    contactEnergy[1]["Value"] = Jcm
                ...
                if j >= dyntime:
                    ...
                    mt1Elements[0]["MembraneMT1"] = membranemt1
                ...
                # print(f'MCS = {j}, active MT1-MMP = {activemt1}, Jcm = {Jcm}')
    ...
    # ##--- Uncomment these for display ---##    
    plt.show(block=False) #--- Comment this for no display ---#
    plt.pause(0.01) #--- Comment this for no display ---#
    plt.close()

    # plt.close(fig) #--- plt.close(fig) for no display; plt.close() for display ---#
